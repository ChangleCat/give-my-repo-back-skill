...
5. **提交证据。** 带回 diff、运行输出、执行路径或观察结果，并注明仓库版本和适用范围。每项检查记录为 `pass`、`fail` 或 `not_run`；`not_run` 不能证明代码正确。
...
这些普通 Markdown 文件记录会话之间的进度，也方便换一个 harness 继续。让新 agent 加载 skill 并从这个目录续接即可。它应核对仓库变化、保留已有卡片编号和人的笔记，从记录的下一步继续。证据会作为学习历史保留；如果相关行为或证据依赖的输入发生实质变化，其有效性从 `current` 改为 `needs_recheck`，无关变更不影响它。只讨论、不保存文件的场景，也可以把材料留在对话中。
